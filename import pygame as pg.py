...
dog_surf = pg.image.load('ELPRIMO.png')
dog_rect = dog_surf.get_rect(bottomright=(750, 800))
...
class Player(pg.sprite.Sprite):
    ...
    def update(self):
        if collision:
            self.rect.x += self.speed_x * 45
            self.rect.y += self.speed_y * 45
        self.rect.x += self.speed_x
        self.rect.y += self.speed_y

        key = pg.key.get_pressed()
        if key[pg.K_a]:
            if self.speed_x != 2 and self.speed_x != -1:
